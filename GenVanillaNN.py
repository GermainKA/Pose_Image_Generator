...
class GenVanillaNN():
    ...
    def train(self, n_epochs=20):
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu') #Exception
        self.netG.to(device) 
        self.netG.train()
        optimizer = torch.optim.Adam(self.netG.parameters(), lr=0.0002, betas=(0.5, 0.999)) #Adam optimizer
        criterion = nn.MSELoss()
        self.netG.apply(init_weights) #init weights
        
        for epoch in range(n_epochs):
            for i, (ske, images) in enumerate(self.dataloader, 0):
                ske = ske.to(device)
                ...
                if i % 30 == 0:
                    print(f"Epoch [{epoch+1}/{n_epochs}], Step [{i}/{len(self.dataloader)}], Loss: {loss.item():.4f}")
